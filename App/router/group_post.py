...
from App.models.group_post import Group, GroupMember, GroupPost
...
@router.delete("/groups/{group_id}/kick/{user_id}")
def kick_member(group_id: int, user_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    admin = db.query(GroupMember).filter_by(group_id=group_id, user_id=current_user.id, is_admin=True).first()
    if not admin:
